...
class _BauCuaRoomScreenState extends State<BauCuaRoomScreen> with SingleTickerProviderStateMixin {
                                    ...
                                    child: SingleChildScrollView(
                                      child: Column(
                                        ...
                                        children: [
                                          ...
                                          if (room.status == 'waiting' && room.hostUid == BauCuaService().currentUser?.uid)
                                            Padding(
                                              padding: const EdgeInsets.only(top: 10),
                                              child: _buildHostStartButton(),
                                            ),
                                          const SizedBox(height: 10),
                                        ],
                                      ),
  ...
  Widget _buildHostStartButton() {
    return Padding(
      ...
      child: ElevatedButton(
        ...
        child: const Text('BẮT ĐẦU XÓC', style: TextStyle(fontWeight: FontWeight.bold, letterSpacing: 1.2)),
      ),
    );
  }
  ...
  Widget _buildStatusBadge(BauCuaRoom room) {
    ...
    switch (room.status) {
      case 'waiting':
        text = isHost ? 'NHẤN BẮT ĐẦU ĐỂ XÓC' : 'ĐỢI NHÀ CÁI...';
  ...
  Widget _buildHistoryBar(List<List<int>> history) {
    if (history.isEmpty) return const SizedBox(height: 10);
    return Container(
      height: 45,
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Text('SOI CẦU', style: TextStyle(color: Colors.amber, fontSize: 8, fontWeight: FontWeight.w900, letterSpacing: 1.0)),
          ),
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return Container(
                  ...
                  child: Row(
                    children: results.map((d) => Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 1),
                      child: _buildHistoryDie(d),
                    )).toList(),
                  ),
                );
              },
  ...
  Widget _buildHistoryDie(int index) {
    return Container(
      width: 14,
      height: 14,
      decoration: BoxDecoration(
