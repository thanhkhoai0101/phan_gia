...
class _TienLenLobbyScreenState extends State<TienLenLobbyScreen> {
  ...
  void _createRoom(BuildContext parentContext, String uid, num userBalance) {
  ...
  void _showQuickPlayForm(BuildContext context, String uid, num userBalance, List<RoomModel> allRooms) {
    String selectedRule = 'per_card';
  ...
  Widget _buildBalanceBadge(UserModel user) {
    return Row(
      ...
      children: [
        ...
        const SizedBox(width: 6),
        Text(user.balance.toCompactFormat(), style: const TextStyle(color: Colors.amberAccent, fontWeight: FontWeight.bold, fontSize: 13)),
      ],
    );
  }
